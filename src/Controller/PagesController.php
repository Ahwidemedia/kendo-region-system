...
class PagesController extends AppController
{
    ...
    public function index()
    {
        ...
        $articles = $this->Evenements->find('all')->contain(['Passages','Competitions'=> ['Disciplines']]);        
        ...
        if($this->Auth->User('id') !== null){
            ...
            $inscriptions = $this->InscriptionCompetitions->find()->where(['user_id'=>$user_id]);
            $number_compete = $inscriptions->count();
            $this->set('number_compete', $number_compete);          
            
            $this->loadModel('InscriptionPassages');
            $inscriptionPassages = $this->InscriptionPassages->find()->where(['user_id'=>$user_id]);
            $number_passage = $inscriptionPassages->count();
            $this->set('number_passage', $number_passage);
        }
    }
    ...
    public function contact(){
        ...
        if($this->request->is(['post'])) {
        	...
        	$CakeEmail->viewVars(['message' => nl2br($data['message']), 'username' => $data['username']]);
