        ...
        <p class="margin-top-3 center">
            ...
            <?php echo $this->Form->input("commentaire", ['type'=>'textarea','class'=>'width-80 center','label'=>false]); ?>
               ...
               <p class="margin-top-3 center width-60 border-red padding-1 notif-qs red">  
                  Les combattants souhaitant se présenter avec un <?php echo $this->Html->link('certificat QS','/files/certificat-qs.pdf', ['target'=>'_blank','style'=>'color:blue; text-decoration:underline']);  ?> doivent obligatoirement fournir l'attestation sur 
                   l'honneur ci-dessous, accompagnée d'un certificat médical de moins de 3 ans mentionnant "apte à la pratique en compétition", sans quoi leur participation se verra refusée :
                    <br />
                   <?php echo $this->Html->image('pdf.jpg',['target'=>'_blank', 'class'=>'center width-5 margin-top-3','url'=>'/files/attestation-certificat-qs.pdf']); ?><br />
                  <span style="color:grey; font-size:0.9em;">Attestation sur l'honneur</span> <br />
                   <br />
                   Pour les autres, le certificat médical mentionnant apte à la pratique en compétition est toujours obligatoire.
                <br /><br />
                   Pour les juniors et espoirs surclassés, la présentation d'un <span style="text-decoration:underline">certificat médical de surclassement</span> est obligatoire.
                     <br /><br />
                   Pour les mineurs, <?php echo $this->Html->link('une autorisation parentale','/files/autorisation-parentale.pdf', ['target'=>'_blank', 'style'=>'color:blue; text-decoration:underline']);  ?> doit être présentée.
               </p>
...
$("#doublon > span").html(newHTML.join(""));
   ...
   $("#doublon").html("<span></span>");}                               
    ...
    $(".lines2 :input").blur(function() {                                       
        ...
        var ddn = row.children().eq(4).children().eq(0).children().eq(0).find(':selected').text();                                   
        var grade = row.children().eq(5).children().eq(0).children().eq(0).find(':selected').text();
        var certificat = row.children().eq(6).children().eq(0).children().eq(0).val();
          ...
          if(licence.length !== 0   || 
            ...
            prenom.length !== 0 
               ...
               row.find("td input:text,td select").prop('required', true);
                         ...
                         } else { row.find("td input:text,td select").prop('required', false); }
